...
#java -classpath ./bin:./third_party/* codeu.chat.ServerMain "100.101" "ABABAB" "2007"
TEAM_ID="$1"
TEAM_SECRET="$2"
PORT="$3"
PERSISTENT_DIR="$4"
...
java -classpath ./bin:./third_party/*

#cd './bin'
if [ "$RELAY_ADDRESS" == "" ] ; then
  java -classpath ./bin:./third_party/* codeu.chat.ServerMain \
      "$TEAM_ID" \
      "$TEAM_SECRET" \
      "$PORT" \
      "$PERSISTENT_DIR"
